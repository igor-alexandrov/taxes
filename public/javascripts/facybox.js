...
;(function($) {
  ...
  $.extend($.facybox, {
				...
				<table> \
					<tbody> \
						...
						<tr> \
							<td class="w" /> \
							<td class="body"> \
							<div class="facybox__footer"> </div> \
							<a href="#" class="close"></a>\
							<div class="facybox__content"> \
							</div> \
						</td> \
							<td class="e"/> \
						</tr> \
    ...
    wait: function(){
      var $f = $('#facybox');
      $('.facybox__content',$f).empty();//clear out old content
	...
	var $f = $('#facybox');
	$('.facybox__content',$f)
		.attr('class',($.facybox.content_klass||'')+' facybox__content') //do not simply add the new class, since on the next call the old classes would remain
		.html(content);
  // $('.loading',$f).remove();
  ...
  function revealImage(href){
    ...
    $('#facybox .facybox__content').empty();
    ...
    image.onload = function() {
		...
		var $footer  	= $("div.facybox__footer",$f);
		var $content 	= $("div.facybox__content",$f);
  ...
  $(document).bind('close.facybox', function() {
	...
	if($.browser.msie){
		...
		$('#facybox').fadeOut('fast',function() {
	      $('#facybox .facybox__content').removeClass().addClass('facybox__content');//revert changing class
	      hideOverlay();
	      $('#facybox .loading').remove();
	    })
	}
